...
describe('JSONTransformerService — Group Nested in Branch', () => {
    ...
    it('should generate correct branch headers when a group is nested inside the branch', () => {
        ...
        const headers = Papa.parse(
            JSONTransformerService.getBranchCSVHeaders({
                branchRef: BRANCH_OWNER_INPUT_REF,
                formRef: FORM_REF
            }, branchWithGroupMappings)
        ).data[0];
            ...
            [FORM_REF]: {
                'branch_owner': {
                    branch: {
                        ...
                        'group_media': {
                            group: {'q_photo': {map_to: 'Photo'}, 'q_video': {map_to: 'Video'}},
                            map_to: 'MediaGroup'
                        },
                        'group_details': {
                            group: {'q_notes': {map_to: 'Notes'}, 'q_rating': {map_to: 'Rating'}},
                            map_to: 'DetailsGroup'
                        }
                    },
                    map_to: 'BranchCount'
